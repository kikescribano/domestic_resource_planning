<p align="center"><picture><source media="(prefers-color-scheme: dark)" srcset="docs/assets/brand-banner-dark.svg"><img src="docs/assets/brand-banner.svg" alt="DRP — El ERP doméstico" width="380"></picture></p>

# DRP · Domestic Resource Planning

> **Estado del documento:** vivo — se actualiza a medida que el proyecto avanza.
> **Última actualización:** 2026-08-21
> **Fase actual:** **Fase 1 (Core MVP) y Fase 2 (Módulos activables) completadas**, el 2026-08-17 y el 2026-08-19. Cómo se hizo cada una, hito a hito, en [`roadmap.md`](docs/common/product/roadmap.md) y [`phase-2-roadmap.md`](docs/common/product/phase-2-roadmap.md). Lo entregado: **el core completo** —54 operaciones— más **el mecanismo de activación por hogar**, **la plataforma que programa las comprobaciones periódicas y entrega los avisos** y **los cuatro módulos de prioridad alta** de la sección 4.2: Proveedores, Warehouse, Compras y Mantenimiento. El contrato queda en **98 operaciones**, el modelo en **28 tablas** con Row-Level Security y la batería en **siete recorridos verticales** sobre un navegador de verdad. **Completado también el [cierre de huecos](docs/common/product/open-gaps-roadmap.md)**, el 2026-08-20 y con sus siete hitos en el día: los cuatro que las dos fases dejaron abiertos a propósito —la baja de un hogar, la conversión de HEIC, el Transactional Outbox y los cuatro atributos propuestos—, más el quinto que apareció ejecutándolos, **el «hoy» de las reglas de calendario, que era el de UTC y no el del hogar**. Lo que queda hecho: un hogar que pide marcharse desaparece entero a los treinta días —sus filas, las de los cuatro módulos y sus ficheros en disco—, una persona puede cerrar su cuenta y llevarse su avatar, **un evento publicado ya no se pierde si el proceso se cae antes de repartirlo**, **la foto que hace un iPhone con los ajustes de fábrica se sube y se ve**, una cosa del hogar puede decir **en qué estado está** —y un préstamo, en qué estado salió y en qué estado volvió—, el inventario se clasifica **por más de una cosa a la vez**, con etiquetas libres y con categorías que tienen icono y color, y **el día que una regla de calendario llama «hoy» es el del hogar y no el de Greenwich**. El contrato queda en **106 operaciones**, el modelo en **31 tablas** y la batería en **diez recorridos verticales**, y con eso quedan cerrados **los cuatro atributos** que 4.1.7 llevaba proponiendo desde el 2026-08-09. No es una fase, y su documento dice por qué. **Y desde el 2026-08-21 DRP corre en producción**: el bloque de despliegue ([ADR-016](docs/common/architecture/decisions/ADR-016-production-deployment.md)) lo puso en el VPS de OVH —compose de producción, imágenes desde GHCR, correo real por una cuenta dedicada y un tope de hogares configurable, con el juego de demostración cargado—; **ese mismo día, en ampliación de la ADR-016, llegaron dominio y TLS**: `https://drp.kikescribano.es`, certificado de Let's Encrypt, HSTS y los ficheros firmados por subdominio. Después va la **Fase 3** —los nueve módulos restantes de 4.2—, que **todavía no está planificada**: convertir una fase en un plan es su propia sesión de trabajo, como lo fue para la 2
...
|---|---|---|---|
| Proveedores y contactos de servicio | Fontanero, servicio técnico de la caldera, taller: quién arregla, quién cobra y quién responde de una garantía. Es dato maestro compartido — sin módulo propio lo duplicarían CMMS, gastos y garantías | **En producción** | Alta |
...
>
> **Desde el 2026-08-21 el despliegue existe** ([ADR-016](docs/common/architecture/decisions/ADR-016-production-deployment.md) y [`deployment.md`](docs/backend/operations/deployment.md)) **y los cuatro pasan a «En producción»**: DRP corre en el VPS, bajo `https://drp.kikescribano.es`, con los cuatro módulos desplegados y operativos en la instancia real. El estado dice eso y no otra cosa — que el módulo funciona donde se usa, no solo que su código está entero. Lo que el despliegue no trae por sí solo son datos de uso: las dos decisiones aplazadas a «la primera revisión de operación» siguen esperando esa revisión, que ahora ya tiene dónde ocurrir.
>
> Que la distinción no es formalismo se vio en este mismo cierre: dos de las decisiones que los módulos habían dejado abiertas —si la categoría de servicio pasa a catálogo por hogar, y si el consumo debe repartirse entre lotes— **preguntan qué eligen los hogares de verdad**, y no se pudieron contestar por eso mismo. Su responsable pasó a ser «la primera revisión de operación con hogares reales dentro», que sigue pendiente (ver 4.1.7): el despliegue le da dónde ocurrir, no la respuesta.
>
...
- **Transactional Outbox: hecho** desde el 2026-08-20 ([ADR-013](docs/common/architecture/decisions/ADR-013-transactional-outbox.md)). Publicar escribe una fila en `event_outbox` **dentro de la transacción del caso de uso**, así que el evento no se pierde si el proceso cae entre el `COMMIT` y el reparto. **La firma de `EventBus` no cambia** y **la garantía tampoco**: sigue siendo at-least-once y los handlers siguen teniendo que ser idempotentes — lo que el outbox añade es que el evento no se pierda, no que llegue una sola vez.
- El reparto **sigue ocurriendo en el acto**, después del commit y antes de responder; el outbox **convive** con él en lugar de sustituirlo, y `OutboxRelay` —periodo propio medido en segundos, hogar a hogar y sin `BYPASSRLS`— es el camino de recuperación de lo que nadie confirmó. La fila entregada **se borra**: la cola vacía es el estado normal y que crezca es el síntoma.
...
| Comunicación interna BE | Event bus (in-process) | Contrato definido (ver 5.2); se implementa como puerto `EventBus` propio sobre el `ApplicationEventPublisher` de Spring, sin dependencia añadida; con **Transactional Outbox** desde el 2026-08-20 para que un evento publicado no se pierda (ver 5.2.2 y ADR-013) |
...
| **Cierre de huecos** (bloque entre fases, **no es una fase**) | Los cuatro que las Fases 1 y 2 dejaron abiertos a propósito: baja de hogar y de cuenta, conversión de HEIC, Transactional Outbox y los cuatro atributos propuestos, más el defecto del «hoy» de las reglas de calendario que apareció ejecutándolos | 🟢 **Completado** el 2026-08-20, con sus siete hitos |
| **Despliegue** (bloque entre fases, **no es una fase**) | DRP en producción en el VPS de OVH: compose de producción, imágenes desde GHCR, correo real, tope de hogares y el juego de demostración cargado — y, en ampliación ese mismo día, **dominio y TLS**: `drp.kikescribano.es` con Let's Encrypt | 🟢 **Completado** el 2026-08-21 ([ADR-016](docs/common/architecture/decisions/ADR-016-production-deployment.md), con su ampliación) |
...
**Alcance:** los cuatro huecos que las Fases 1 y 2 dejaron **abiertos a propósito**, cada uno con su motivo escrito y —cuando lo tenía— su destinatario: la **baja de un hogar** y el borrado de sus ficheros, con la del avatar al cerrar la cuenta dentro porque resultó ser la misma pregunta; la **conversión de HEIC**, que hacía que la foto de un iPhone con los ajustes de fábrica se rechazara con un `415` —**cerrada el 2026-08-20**: la convierte el cliente, como 5.8.3 ya decía, y sin que la lista blanca del servidor crezca—; el **Transactional Outbox** que nombra 5.2.2; y los **cuatro atributos propuestos** —estado de conservación, condición en préstamo, etiquetas libres, e icono y color de categoría—, **los cuatro hechos desde el 2026-08-20**: los dos primeros sin ADR y sin tocar el contrato, y los dos segundos con la ADR-015, dos tablas más y cuatro operaciones.

**Y un quinto que no estaba en ninguna lista**, porque no se dejó abierto sino que apareció ejecutando el bloque (2026-08-20): **el «hoy» de las reglas de calendario era el de UTC y no el del hogar**, así que un hogar peninsular no podía registrar de madrugada lo que acababa de hacer. Entra por el mismo motivo que los otros cuatro —es saldo de las dos fases cerradas, no avance del producto— y ocupa el Hito 5, **cerrado el 2026-08-20**: el día es el del hogar, resuelto con un puerto de plataforma que el core implementa y sin migración, sin operación nueva y sin ADR nueva —la ADR-011 se amplía.

**No es una fase, y por eso no tiene número.** Se ejecuta como una —siete hitos, una sesión y un pull request por hito—, pero la numeración de fases describe el avance del producto y esto es saldo de lo que las dos primeras dejaron a deber. Se descartaron las dos alternativas: hacerlo **hito 0 de la Fase 3**, que obligaría a planificar esa fase para poder empezar por su hito 0, y **numerarlo como fase propia**, que exigiría renumerar los nueve módulos en tres documentos para decir algo que no es cierto.
...
**Completado el 2026-08-20, con sus siete hitos en el día.** El último no añadió producto: consolidó, como el de la Fase 2 y por la misma lección — lo que se deja para «al final de otro hito» no se cierra. La capacidad está vuelta a medir con las tablas y los atributos del bloque dentro —VPS-3 sigue en pie y sigue decidiéndolo el disco—, la auditoría sistemática de accesibilidad cubre **todas las pantallas de la navegación** —Sitios y Personas eran las dos últimas del core sin auditar—, y el contrato queda en **106 operaciones**, el modelo en **31 tablas** y la batería en **diez recorridos verticales**.
...
>
> El bloque añade **cuatro ADR** a las once existentes, y **las cuatro están escritas**, cada una en el hito que la estrenó y no al planificar: supresión de datos ([ADR-012](docs/common/architecture/decisions/ADR-012-data-erasure-household-closure-and-account-closure.md), Hito 0), Transactional Outbox ([ADR-013](docs/common/architecture/decisions/ADR-013-transactional-outbox.md), Hito 1), conversión de HEIC ([ADR-014](docs/common/architecture/decisions/ADR-014-heic-conversion.md), Hito 2) y color e icono elegidos por el usuario dentro de un juego certificado ([ADR-015](docs/common/architecture/decisions/ADR-015-user-chosen-category-identity.md), Hito 4). **El Hito 3 no tenía ninguna asignada y no se le inventó una**: tres columnas anulables sobre dos tablas que ya existen no dejan ninguna alternativa estructural que descartar, así que sus ocho decisiones están en 4.1.7 y no en una ADR de trámite. El Hito 4 sí la tiene y por el motivo contrario: descartar el selector libre de color cambia lo que el sistema de diseño garantiza. **El Hito 5 tampoco tiene ADR propia** y por la misma razón que el 3: la inversión con un puerto de plataforma ya estaba decidida y usada dos veces, así que lo que hacía falta no era una decisión estructural sino matizar una frase — y la [ADR-011](docs/common/architecture/decisions/ADR-011-scheduled-checks-and-notice-delivery.md) gana su sección hacia adelante en lugar de reescribirse.
>
...
### 8.5 Despliegue (bloque entre fases)

**Alcance:** DRP corriendo en producción, en su forma mínima viable y con lo aplazado dicho por escrito. Un bloque y un pull request, como los hitos de siempre, y **tampoco es una fase**: no avanza el producto — lo pone donde se usa.

Lo que entrega: el **compose de producción** (`deploy/`) con PostgreSQL, el backend y nginx sirviendo la SPA y los ficheros firmados desde su otro origen; las **imágenes construidas por la CI y publicadas en GHCR**, de modo que desplegar es `pull` y volver atrás es fijar un SHA; los **secretos generados en el servidor** y solo allí; el **correo real** por una cuenta de Gmail dedicada; el **programador y el relay encendidos** con una sola instancia (ADR-011); el **tope de hogares** de `CreateHousehold` —la única pieza de código del bloque, con su recorrido de pruebas en las tres capas—; y el **juego de demostración cargado en la instancia real**. El bloque se cerró dejando fuera dominio y TLS a propósito, **y los dos llegaron ese mismo día como ampliación de la ADR-016**: `kikescribano.es` comprado (el `.ovh` previsto resultó estar suspendido para registros nuevos), certificado de Let's Encrypt con renovación sin cortes, HSTS encendido y la separación de ficheros de la ADR-005 mudada del puerto 8081 a `files.drp.kikescribano.es`, que era su forma definitiva. Queda deliberadamente fuera, cada cosa con su motivo: **el análisis antivirus y el despliegue continuo** — este último anotado como deuda con su forma ya elegida en el manual.

**Completado el 2026-08-21.** La decisión entera, con sus alternativas descartadas, en la [ADR-016](docs/common/architecture/decisions/ADR-016-production-deployment.md); el manual de operación —desplegar, revertir, restaurar—, en [`deployment.md`](docs/backend/operations/deployment.md), y **solo allí**.
...
| Auditoría de seguridad (OWASP) previa al despliegue | [`common/security/owasp-audit.md`](docs/common/security/owasp-audit.md) |
| Despliegue de producción (VPS, versiones, restauración) | [`backend/operations/deployment.md`](docs/backend/operations/deployment.md) |
...
| Fecha | Cambio |
|---|---|
| 2026-08-21 | **Los cuatro módulos de prioridad alta pasan a «En producción» (4.2)**: Proveedores, Warehouse, Compras y Mantenimiento (CMMS). Con el despliegue completado y el dominio con TLS ese mismo día ([ADR-016](docs/common/architecture/decisions/ADR-016-production-deployment.md) y su ampliación), los cuatro corren desplegados y operativos en la instancia real de `https://drp.kikescribano.es`, y la tabla pasa a decirlo; sus notas se actualizan en consecuencia. Lo que no cambia: las dos decisiones aplazadas a «la primera revisión de operación con hogares reales dentro» siguen esperándola (ver 4.1.7). Pendiente derivado, por la regla de la sección 11: las presentaciones que declaran «En desarrollo» se regeneran antes del siguiente envío |
| 2026-08-21 | **Dominio y TLS: DRP pasa a `https://drp.kikescribano.es`**, en ampliación de la [ADR-016](docs/common/architecture/decisions/ADR-016-production-deployment.md) y el mismo día que el bloque que los dejaba fuera. El plan A —un `.ovh` de ~3 €— resultó imposible: **OVH tiene suspendido el registro de `.ovh` nuevos** (y `drp.es` está cogido), así que el dominio es `kikescribano.es` con DRP en su subdominio; el autofirmado se descartó porque HTTPS con aviso a toda página no es producción. El certificado es **de Let's Encrypt y uno con los dos nombres** —aceptación del Subscriber Agreement autorizada expresamente, con el correo del proyecto—, emitido en standalone la primera vez (el 443 no podía arrancar sin él) y renovado **sin cortes** por webroot: el desafío ACME vive en `deploy/data/acme`, nginx lo sirve en un puerto 80 que ya solo redirige, y un gancho recarga nginx al renovar. **La separación de ficheros de la ADR-005 llega a su forma definitiva**: `files.drp.kikescribano.es` por SNI en el 443, y **el puerto 8081 deja de existir**; el **HSTS** que la plantilla llevaba escrito y comentado desde el endurecimiento OWASP se enciende, y `forward-headers-strategy: native` hace que el backend diga `isSecure()` con verdad. La plantilla de nginx queda en cuatro servidores —dos del 80 para ACME y redirección, dos del 443 repartidos por nombre—, el compose publica 80 y 443 y monta `/etc/letsencrypt` en solo lectura, y el `.env` gana `DRP_APP_HOST`/`DRP_FILES_HOST` con sus URL en `https`. El manual gana el arranque en frío con el certificado **antes** que nginx y dos síntomas nuevos en su tabla |
| 2026-08-21 | **Banner de marca en la cabecera del README**: el documento se abre con la marca de DRP centrada —el sello redondo con la casa, «DRP» y el lema—, tal y como la pintan las pantallas sin sesión de la aplicación (`BrandMark` con `size="hero"`). Son dos SVG autocontenidos nuevos en `docs/assets/` —modo claro y modo oscuro, elegidos con `<picture>`—, con «DRP» trazado como paths de Baloo 2 extrabold (un SVG mostrado vía `<img>` no puede cargar fuentes externas) y los colores convertidos a hex sRGB desde los tokens OKLCH de `frontend/src/index.css` |
| 2026-08-21 | **El despliegue continuo queda anotado como deuda, con su forma ya elegida.** Desplegar sigue siendo deliberadamente manual —fusionar publica las imágenes; el `pull` en el VPS lo ejecuta alguien— y esta anotación deja escrito el día que eso cambie: un job de SSH **en el propio workflow de imágenes**, con clave dedicada de **`command=` forzado** (un secreto comprometido solo podría desplegar, nada más), `concurrency`, abstención ante un `DRP_VERSION` anclado y fuera de la ventana del barrido de las 03:15. Se descarta el agente de sondeo en el VPS, que despliega a destiempo y no actualiza el checkout de `deploy/`. La [ADR-016](docs/common/architecture/decisions/ADR-016-production-deployment.md) gana el apunte en su lista de abiertos y [`deployment.md`](docs/backend/operations/deployment.md) la opción entera; ejecutarlo será su propio bloque pequeño |
| 2026-08-21 | **Despliegue de producción: DRP corre en el VPS de OVH**, en `http://vps-7f6cfe1b.vps.ovh.net/`, y el despliegue deja de ser «una decisión que nadie ha tomado». Es un bloque entre fases —no una fase, no avanza el producto: lo pone donde se usa— con la [**ADR-016**](docs/common/architecture/decisions/ADR-016-production-deployment.md) como decisión y [`deployment.md`](docs/backend/operations/deployment.md) como manual, y la sección **8.5** nueva. Lo que entrega: un **compose de producción** (`deploy/`) con PostgreSQL, backend y nginx sirviendo la SPA y proxyando `/api` —solo nginx publica puertos, y queda escrito que **los puertos de Docker puentean UFW**, así que esa lista ES el cortafuegos—; los ficheros firmados desde **su otro origen, el puerto 8081**, porque sin dominio la separación de la ADR-005 se hace por puerto; las **imágenes construidas por la CI y publicadas en GHCR** con etiqueta `latest` y SHA —desplegar es `pull`, volver atrás es fijar un SHA en el `.env`—; los **secretos generados en el servidor** y nunca en el repo, con los roles de la ADR-003 aprovisionados por un init parametrizado que espeja el canónico; el **correo real por una cuenta de Gmail dedicada** con contraseña de aplicación —la pieza que la ADR-009 dejó preparada: sustituir Mailpit es configuración—; el **programador y el relay encendidos con una sola instancia** (ADR-011); y el **juego de demostración cargado en la instancia real**, con sus dos costes asumidos por escrito. La única pieza de código es el **tope de hogares**: `CreateHousehold` gana `maxHouseholds` (`DRP_MAX_HOUSEHOLDS`, cero es sin tope, el valor real vive solo en el `.env` del servidor) y responde **`409 HOUSEHOLD_LIMIT_REACHED` antes de tocar nada e idéntico exista o no el correo** —para no reabrir por la puerta del tope el oráculo que el `202` constante cierra—, con recorrido vertical completo: código en el enumerado y en el contrato —`createHousehold` gana su `409`, sin operación nueva—, prueba unitaria que afirma que la rama del tope **no hashea, no busca, no escribe y no envía**, prueba de integración por HTTP real cuyo tope se calcula contando la base compartida, y el mensaje en la pantalla de alta con su prueba de componente. Tres comprobaciones previas condicionaron el bloque y quedaron escritas: **HTTP plano no rompe nada del frontend actual** —auditoría de contexto seguro limpia: sin escáner de cámara, y la conversión HEIC no usa API restringidas—, **el entorno tiene precedencia sobre `application.yml`** —producción se configura sin tocar código—, y **las migraciones presuponen los roles** —V5 falla sin `drp_resolver`—. Los estados de 4.2 y la sección 8 **no cambian de «En desarrollo», y el motivo se muda**: ya no es «no hay despliegue» sino «no hay ningún hogar real dentro». Deliberadamente fuera, cada cosa con su motivo en la ADR: **dominio, TLS** —hasta el escáner de códigos o el primer hogar ajeno— y el **análisis antivirus** de siempre |
| 2026-08-20 | **Reactivación de miembros del hogar**: la baja de una pertenencia gana su vuelta, y con ella la pantalla «Personas» gana el interruptor de Módulos. Es un bloque propio entre el cierre de huecos y la Fase 3 —no avanza los nueve módulos, completa un ciclo del core que estaba a medias: la baja existía y no tenía deshacer—. Entra **`ReactivateUser`** (`POST /users/{id}/activation`, solo `HOUSEHOLD_ADMIN`), la misma pareja subrecurso que los módulos y **no** un renombrado del `DELETE /users/{id}` ya publicado; **idempotente con `200`** como `activateModule`, eligiendo a propósito entre los dos criterios que el repo ya tenía. El contrato pasa de 106 a **107 operaciones** con **ninguna migración**: el modelo sigue en 31 tablas, porque la vuelta es limpiar `deactivatedAt` en la fila que ya estaba. Lo que la vuelta **no** deshace, decidido y escrito: los refresh tokens revocados no vuelven y los assets huérfanos no se reasignan solos; el rol se conserva, porque la fila es la misma. Dos `409` la acotan: `IDENTITY_ALREADY_MEMBER` reutilizado —el índice único parcial de la pertenencia activa respondería con un `500`, y la comprobación usa la función acotada `find_household_for_active_member`, sin `BYPASSRLS`— y **`IDENTITY_CLOSED`, código nuevo**: reactivar la pertenencia de una cuenta cerrada fabricaría un miembro activo que no puede entrar nunca. La implementación destapó dos deudas y las salda: **re-invitar a un ex-miembro violaba `UNIQUE (household_id, identity_id)`** —un `500` inalcanzable mientras ninguna pantalla diera de baja a nadie— y ahora la aceptación **revive** la pertenencia con el rol de la invitación, como una etiqueta retirada; y **`UserDeactivated` se publica por fin** —el contrato lo prometía desde la Fase 1 y el código no lo emitía— desde las dos bajas de pertenencia, `DeactivateUser` y `CloseAccount`. La vuelta **no** publica evento: el criterio de 5.2.3 es la necesidad de un módulo y no la simetría, y su destinatario —el planificador de tareas— queda anotado. En pantalla, el interruptor solo lo ve quien administra —que ahora lista con `includeDeactivated=true`, lo único que permite encontrar a quien traer de vuelta—, **apagar arma una confirmación en la propia fila** —revoca sesiones en el acto; no es un modal, por el criterio de la ficha de `DangerZone` y sin estrenar el `Dialog` pendiente— y **la propia fila no lleva interruptor**. El barrido de aislamiento pasa de 44 a **46 operaciones** con identificador en ruta —entra la nueva y entra `DELETE /users/{id}`, que llevaba desde la Fase 1 fuera— y el retrato del hogar A gana su listado de personas con los dados de baja dentro; la baja gana además **su primera prueba de camino feliz**, que no existía. El recorrido vertical pasa de diez a **once**: el nuevo estrena el flujo de invitación en navegador real —la invitada acepta y elige contraseña en un contexto sin sesión—, mide que la baja corta la sesión de verdad y que la vuelta se hace con las credenciales de siempre, y audita la variante de «Personas» con interruptor y confirmación armada que la pasada sistemática no ve. Sin ADR, por el criterio del Hito 3 del cierre de huecos; las decisiones, en 4.1.7 |
| 2026-08-20 | **Auditoría de seguridad OWASP previa al despliegue**, en la nueva [`docs/common/security/owasp-audit.md`](docs/common/security/owasp-audit.md) —directorio `common/security/` estrenado, porque la auditoría cruza backend, frontend e infraestructura y no cabe en `backend/security/`—. Contrasta el código y la configuración reales contra las OWASP Cheat Sheets sobre las 106 operaciones y 31 tablas del cierre de huecos, en cinco superficies: autenticación y sesiones, autorización y multi-tenant, ficheros, inyección/XSS/cabeceras, e infraestructura y dependencias. **Veredicto: ninguna vulnerabilidad crítica y una base notablemente sólida** —los dos invariantes de aislamiento fijados por pruebas, Argon2id a los parámetros de OWASP, tokens de un solo uso hasheados, login sin enumeración, entrega de ficheros que sigue la File Upload Cheat Sheet—. Lo que hay que cerrar antes de exponerlo son **cinco hallazgos ALTA**, tres de ellos *promesas que el propio código o la documentación hacen y quedaron a medias*: el rate limit por IP que colapsa en un cubo global detrás del nginx que el repositorio ya despliega —porque la lectura de `X-Forwarded-For` que un comentario prometía «cuando llegue nginx» nunca se escribió—, el fail-open del secreto JWT sin perfil activo y el del secreto de firma de ficheros sin validar, `webp-imageio 0.1.6` con libwebp anterior al fix de CVE-2023-4863 decodificando WebP subido, y Spring Boot 3.4.x fuera de soporte OSS sin detección continua de dependencias. Catorce **MEDIA** más —re-autenticación del cierre de cuenta, sesión sin tope absoluto, refresh en `localStorage`, CSP y cabeceras de la SPA, CORS de producción, PDF servido con URL de portador—, y los **requisitos del despliegue** —TLS, compose endurecido, firewall, backups automatizados— listados como bloque propio para que entren en su documento de alcance. El detalle, con `fichero:línea` y la acción de cada hallazgo, en el documento |
| 2026-08-20 | **Cierre de huecos completado (Hito 6, cierre del bloque)** — y con él, el bloque entero: siete hitos y siete pull requests en dos días sobre lo que las Fases 1 y 2 dejaron abiertos a propósito, más el defecto que apareció ejecutándolos. El hito no añade producto: consolida, como el Hito 6 de la Fase 2 y por la misma lección. La **capacidad está vuelta a medir** con lo que el bloque añadió dentro —las dos tablas del Hito 4, el estado de conservación y las categorías con cara en la siembra—: la pendiente por hogar pasa de 116 394 a **141 994 B** y la de actividad de 2457 a **2525 B/día**, con **VPS-3 en pie y decidiéndolo el disco**, el criterio de retención intacto y una precisión que faltaba: el tamaño físico de `event_outbox` es la huella de su pico, no lo que retiene, porque la cola se vacía y PostgreSQL no devuelve las páginas. La lista de la auditoría de accesibilidad se renombra a **`AUDITED_SCREENS`** —`PHASE_TWO_SCREENS` era falso por partida doble y el rótulo contaba diez pantallas donde había once— **y entran «Sitios» y «Personas»**, las dos últimas del core sin auditar, con lo que **la pasada sistemática cubre la navegación entera: trece pantallas**. Los punteros hacia adelante de `roadmap.md` y `phase-2-roadmap.md` pasan de «hay un plan» a «está hecho» sin reescribir lo que aquellas fases decidieron; `decisions.md` se comprueba y estaba completo, con las cinco preguntas del bloque resueltas en su hito; **`CLAUDE.md` deja de describir el repositorio de antes del bloque** —98 operaciones, 28 tablas, siete recorridos y nueve ADR donde hoy son 106, 31, diez y quince—; y las **cuatro piezas del deck de marketing** se regeneran desde sus generadores con las cifras verificadas contra el repositorio, la atribución de Slidesgo conservada y la decisión de la pieza comercial —no contar el bloque, que es saldo interno— mantenida por escrito. El detalle de los hitos vive en [`open-gaps-roadmap.md`](docs/common/product/open-gaps-roadmap.md), que queda cerrado y se conserva como historia |
| 2026-08-20 | **Cierre de huecos, Hito 5 completado**: el día que una regla de calendario llama «hoy» pasa a ser **el del hogar** y no el de Greenwich. Es el único hito del bloque que **no venía del plan** —apareció ejecutando el Hito 1— y el más pequeño de todos: **sin migración, sin operación nueva, sin pantalla nueva y sin ADR nueva**, con el modelo quieto en 31 tablas y el contrato en 106 operaciones. **La pregunta se decide por lo que significa el tipo** y no por venir recomendada en el plan: `performedOn` y `nextDueOn` son `LocalDate`, y un día de calendario no significa nada sin una zona; **UTC parece la opción neutral y no lo es**, porque es la zona del despliegue, y su precio es que un hogar peninsular no pueda registrar entre la medianoche local y la de Greenwich lo que acaba de hacer —`400`, «no puede ser del futuro», sobre la fecha que tiene delante—. La decisión del 2026-08-17 sobre el vencimiento de un préstamo **no se reabre**: sigue siendo cierta y por su motivo, porque `dueAt` es un `timestamptz` y comparar instantes no necesita zona; la frontera real era `Instant` frente a `LocalDate`. **No nace una ADR y eso es parte de la decisión**: la [ADR-011](docs/common/architecture/decisions/ADR-011-scheduled-checks-and-notice-delivery.md) se amplía con una sección hacia adelante que separa lo que su sección 7 juntaba —**a qué hora corre el recorrido es del despliegue; qué día mira, del hogar**— sin tocar su cuerpo. La respuesta es la inversión que esa misma ADR ya usa dos veces: **`HouseholdCalendar`, declarado en plataforma e implementado por el core** leyendo `households.time_zone`, con la lista de excepciones de ArchUnit intacta en un solo nombre; se descarta llevar la zona en `SessionClaims`, que la engordaría y **no sirve donde más falta hace** —el recorrido diario no tiene sesión—, y se descarta cambiar la zona del bean `Clock`, que solo funcionaría con un hogar por instancia. Con las tres líneas idénticas se va el `?: ZoneId.systemDefault()`, que era **código muerto** y prometía un respaldo imposible; el puerto, en cambio, falla ruidosamente sin contexto de inquilino, porque un respaldo a UTC daría un día plausible y equivocado. En el cliente cambian `today()` y `inMonths()` —que sumaba meses en local y formateaba en UTC— y **un cuarto sitio que el plan no había visto**: `dueStatus()`, que restaba un día de calendario menos `Date.now()` y pintaba «Se ha pasado» sobre un plan que toca hoy. La regresión se fija **con el reloj parado** —23:30 UTC, hogar en `Europe/Madrid`—, comprobada dando `400` sin el cambio, y con dos pruebas que la suite no tenía: una que afirma que el instante elegido **de verdad cambia de día** —para que mover la hora no deje las demás pasando sin medir nada— y otra que ningún hogar solo puede dar, **el mismo instante con dos hogares y dos días**, donde Madrid acepta la fecha que Honolulu rechaza. El recorrido vertical de CMMS gana el caso y con él lo único que el backend no puede decir: **desde un navegador que está en otro día, el campo sigue enseñando el del hogar**. Se corrigen las **tres** frases que justificaban `timeZone` por «el proceso diario de vencidos» —el plan había localizado una, y estaban también en el modelo de datos y en el propio contrato—, y el hogar de demostración gana una línea, porque sus `CURRENT_DATE` salían de la zona del servidor y no de la del hogar. Cinco decisiones que la definición no preveía (ver 4.1.7) |
| 2026-08-20 | **Cierre de huecos, Hito 4 completado**: el inventario se clasifica **por más de una cosa a la vez** y las categorías tienen cara. Entran los **dos atributos que quedaban** de los cuatro que 4.1.7 llevaba fuera desde el 2026-08-09 —**etiquetas libres** e **icono y color de una categoría**—, con lo que esa propuesta queda **cerrada entera**. Trae la [**ADR-015**](docs/common/architecture/decisions/ADR-015-user-chosen-category-identity.md), que **extiende la ADR-006 sin reescribirla** y le añade su sección hacia adelante; la migración `V17`, que lleva el modelo de 29 a **31 tablas**; y **cuatro operaciones**, que lo llevan de 102 a **106**. **La pregunta del hito se decide con las tres cosas medidas y no con una preferencia**: la etiqueta es un **catálogo por hogar** y no una columna de texto porque el texto no se puede renombrar de una vez —habría que recorrer todos los assets de la casa—, ni deduplicar sin distinguir mayúsculas ni acentos —no hay ninguna fila común donde poner la restricción, así que «Camping» y «camping» convivirían—, ni autocompletar sin un `DISTINCT` sobre un campo repetido tantas veces como cosas tenga la casa. Las tres tienen su prueba. **Etiquetar no gana una quinta operación**: viaja en `tagIds` dentro de `POST /assets` y `PATCH /assets/{id}`, absoluto como la cantidad de una existencia, porque una operación propia sería una segunda escritura sobre el inventario para hacer lo que el `PATCH` ya hace. El **icono y el color se eligen dentro de un juego cerrado** —dieciséis y seis— y esa es la ADR entera: un color elegido con un selector libre **no está en ningún token**, así que no lo mide `scripts/check-contrast.py` y sería lo único de la interfaz cuyo contraste se afirma en vez de comprobarse; el script pasa de **36 pares a 48**, y su comprobación de gamut destapó que **tres de los doce valores nuevos nacían fuera de sRGB**. Se descarta el selector libre —que obliga a no certificar nada o a calcular la tinta automáticamente, con lo que el usuario acaba sin el color que eligió— y también la variante de admitirlo avisando, que cambia una garantía por una recomendación. Se cierra además la decisión abierta de **`lucide-react`**, con el número delante: los dieciséis iconos cuestan **5,88 kB sobre la primera carga, 2,63 comprimidos**, aproximadamente lo mismo que dibujarlos a mano, así que lo que decidía no era el peso sino que un juego **adoptado y no instalado** obliga a imitar a mano lo ya elegido; los cuatro iconos que llevaban dos fases dibujados se migran en un commit propio. Detrás aparece una contradicción que la dependencia tapaba y que **no se resuelve aquí**: `iconography.md` declara normativa la tabla de un icono por estado y `status-badge.md` declara lo contrario, con su motivo. **«Catálogo» entra en la auditoría sistemática** —queda una de las cuatro pantallas del core, más Sitios— con una categoría de color puesto sembrada antes, y el recorrido vertical **mide el contraste ya aplicado** leyéndolo del navegador en los dos modos, que es lo único que demuestra que el par medido es el que llega a la pantalla. Lo que la implementación destapó: el índice único de `tags` **no puede ser parcial por retirada** como el de `categories` —allí es inofensivo porque un asset tiene una sola categoría, y aquí dos etiquetas iguales pintarían la misma fila dos veces—, y lo que lo sustituye es que **crear una que existe retirada la revive**, que de paso le da deshacer a la retirada; **Chrome conserva `oklch()` en `getComputedStyle`**, lo que hizo que la primera versión del medidor devolviera 1,00:1 para todo; y dos defectos de accesibilidad que solo aparecieron al escribir las pruebas —JSX se come el espacio inicial de la línea, así que un `sr-only` detrás de un rótulo producía «EditarAlimentación», y el selector puede estar **dos veces en la misma pantalla**, con veintidós nombres repetidos—. La categoría **no entra en la navegación móvil**, que el plan pedía, porque en la navegación no hay ninguna categoría; a cambio se cumple la promesa que `iconography.md` llevaba desde la Fase 1: el marcador de una foto que falta es el icono de su categoría y no un rectángulo gris. Once decisiones que la definición no preveía (ver 4.1.7) |
| 2026-08-20 | **Cierre de huecos, Hito 3 completado**: las cosas del hogar aprenden a decir **en qué estado están**. Entran los **dos primeros de los cuatro atributos** que 4.1.7 llevaba fuera desde el 2026-08-09 —el **estado de conservación** de un asset y la **condición en la entrega y la devolución** de un préstamo— y entran como lo que son: **tres columnas anulables** en la migración `V16`, **ninguna tabla nueva** —el modelo se queda en 29—, **ninguna operación nueva** —el contrato se queda en 102, que era la condición del plan— y **ninguna ADR**, porque no hay alternativa estructural que descartar y escribir una habría sido el trámite en vez de la decisión. Lo que sí trae son **ocho decisiones de producto** (ver 4.1.7). **Una sola escala de cinco valores** —`NEW`, `GOOD`, `WORN`, `DAMAGED`, `UNUSABLE`— para los tres campos, porque el motivo entero de la condición en préstamo es poder decir «salió bien y volvió rayado» y con dos escalas distintas esa frase no se construye. **Solo sobre un `DURABLE`**, como el número de serie y por el mismo motivo —describe una unidad física, y lo que le pasa a un lote es de Warehouse—, ampliando la restricción que ya existía en vez de añadir una segunda. Y la que más se discutió: **anotar cómo volvió algo no cambia la ficha del asset**. Es tentador —el taladro volvió rayado, luego el taladro está rayado— y se descarta porque quien confirma la devolución puede ser una persona **ajena al hogar** con su token acotado, de modo que propagarla le daría una escritura sobre el inventario que su credencial no alcanza; CMMS ya había sentado el precedente al declarar que registrar una intervención «no toca el asset». La devolución gana un **cuerpo opcional** en lugar de una operación nueva, y esa es la otra cara de lo mismo: una operación más sería una **segunda escritura que el token acotado tendría que alcanzar**, cuando hoy alcanza exactamente dos y esa lista es lo que lo contiene. Hay una prueba que manda por esa rendija cinco campos de más —notas, estado, `assetId`, autoría y plazo— y comprueba **desde dentro de casa** que ninguno se escribió. La vista externa del préstamo **se ensancha por primera vez** desde que se escribió, de siete campos a nueve, con su motivo dicho: las dos condiciones describen la cosa que quien pregunta tiene en las manos, no el hogar que se la prestó. En pantalla, **la condición no se pinta con color**: la ficha de `StatusBadge` declara antiuso dos distintivos en la misma fila y la paleta de dominio tiene cinco tonos elegidos para sobrevivir a una deuteranopia, así que un sexto para una escala de cinco valores sería color como único portador — y por eso `check-contrast.py` sigue midiendo **36 pares** y no 46. **«Inventario» entra en la auditoría sistemática** con un asset sembrado, que era una de las cuatro pantallas del core sin auditar, y **el hogar de demostración trae las tres columnas puestas** —con diez duraderos sin anotar a propósito y ninguno `NEW`, porque lo más joven de esa casa tiene once meses—. De paso se destapa algo que no era del hito: **la pantalla de Préstamos era el único fichero del frontend pintado con tokens que no existen** —`text-muted` y `border-line` no generan ni una regla de CSS—, así que su texto secundario salía a plena tinta y sus tarjetas llevaban un borde en `currentColor`. No fallaba nada; se veía como una pantalla que no se parece a las demás |
| 2026-08-20 | **Cierre de huecos, Hito 2 completado**: la **conversión de HEIC**, que era la carencia más visible para el usuario más probable de una aplicación doméstica — la foto que hace un iPhone con los ajustes de fábrica se rechazaba con un `415` que enumeraba cuatro tipos que quien la hizo no eligió. Formalizada en la [ADR-014](docs/common/architecture/decisions/ADR-014-heic-conversion.md), y escrita **después** de las dos medidas que el plan exigía delante, que es la parte que más se nota: **las dos cambiaron el resultado esperado**. El bundle no eran los 402 kB de referencia sino **407,28** —volver a medirlo era la condición, y entre planificar y ejecutar habían entrado dos pantallas—, y sobre todo **el megabyte no cae sobre el bundle**: en un `import()` dinámico el decodificador sale en un fragmento propio de **2 995 kB** que solo descarga quien elige un HEIC, y sobre la primera carga de todo el mundo cuesta **2,49 kB**. El «cambio de categoría» que el plan temía era cierto para un supuesto que resultó evitable. Enfrente, el camino del servidor tampoco era lo que parecía: **no empezaba por escribir código** sino por subir el toolchain de la JVM —el único plugin de ImageIO para HEIF de Maven Central exige **JDK 22** y el proyecto está en 17— y meter `libheif` en la imagen de despliegue y en el runner; y decodificar cuesta de **×3,4 a ×5,6** sobre la operación que ya era la cara por un orden de magnitud, o sea 2,6–4,3 s de núcleo por foto sobre 2 vCPU. **La tercera salida se evalúa y se descarta por escrito**, que era la otra condición: pedirle al usuario que cambie el ajuste de la cámara cuesta cero y pierde porque no arregla la foto ya hecha, no toca el HEIC que llega de un álbum compartido y se paga en la cuota del usuario, ya que JPEG a igual calidad ocupa el doble. Gana el cliente, **que es lo que 5.8.3 ya decía**, así que esa sección no se enmienda sino que se confirma, y la ADR-005 gana una sección que enlaza hacia adelante sin que se toque su cuerpo. La trampa que el plan anunciaba se resolvió del otro lado: **HEIC no llega nunca a `files.content_type`** —lo que se guarda es el tipo detectado tras recodificar—, así que **no hay migración, ni operación nueva en el contrato, ni entrada nueva en el enumerado del dominio**: el modelo sigue en 29 tablas y el contrato en 102 operaciones. Lo que sí crece es la lista del `accept`, que es otra cosa: la de lo que se puede elegir. La conversión vive en `uploadFile` y no en el campo de subida, porque hay **dos** vías de subida —el campo y el avatar, con su propio `<input>`— y en la única puerta por la que pasan las dos, la tercera que alguien escriba nace convertida. La medición de capacidad gana **una tercera magnitud**: lo que cuesta llegar al navegador. Y el recorrido vertical sube **un HEIC de verdad y no un JPEG renombrado** —marca `heic`, y con **352 B de EXIF con coordenadas GPS dentro**, porque con una foto sin metadatos no se distinguiría entre haberlos borrado y no haber tenido ninguno—, lo ve en la galería y en la ficha del asset, y comprueba en los bytes servidos que no sale ni `Exif` ni `GPS`. De paso se destapa algo ajeno al hito y de la Fase 1: **`UploadField` documenta un botón de Cancelar que nunca se construyó**, con su `AbortController` declarado y sin usar y un estado inalcanzable. No se arregla aquí, pero se deja de afirmar. Diez decisiones que la definición no preveía (ver 4.1.7) |
| 2026-08-20 | **El cierre de huecos gana un séptimo hito, y no lo trajo el plan sino la ejecución.** Al pasar la suite de madrugada se destapó que **las tres reglas de calendario del proyecto resuelven su «hoy» contra la fecha UTC y no contra la del hogar** —la que rechaza una intervención «del futuro» en CMMS, y las dos comprobaciones periódicas de CMMS y Warehouse—, con la misma línea copiada en los tres sitios y con el `Clock` de la aplicación en `systemUTC()`. Para un hogar peninsular eso significa que **entre la medianoche local y la de Greenwich no puede registrar lo que acaba de hacer**: su fecha de hoy es futuro para la aplicación. El frontend arrastra el mismo criterio y empeora el síntoma, porque el campo sale relleno con **ayer** y el selector no ofrece hoy, así que el día equivocado se guarda en silencio en vez de dar error. La frontera que esto enseña **no es «CMMS» sino `LocalDate` frente a `Instant`**: comparar instantes no necesita zona —y por eso la decisión de 2026-08-17 sobre el vencimiento de un préstamo sigue intacta—, pero derivar un día de calendario del reloj siempre la necesita. Entra como **Hito 5** del [cierre de huecos](docs/common/product/open-gaps-roadmap.md) y no dentro de su hito de cierre, porque cambia una regla de dominio y aquel no añade producto; el de cierre pasa a ser el **6**. La pregunta que lo decide —día del hogar o día de UTC— **queda anotada, no resuelta**, con la recomendación y su alternativa delante |
| 2026-08-20 | **Cierre de huecos, Hito 1 completado**: llega el **Transactional Outbox** que 5.2.2 nombraba como candidato de evolución desde la Fase 0 ([ADR-013](docs/common/architecture/decisions/ADR-013-transactional-outbox.md)). Publicar escribe una fila en `event_outbox` **dentro de la transacción del caso de uso**, así que entre el `COMMIT` y el `AFTER_COMMIT` que reparte ya no hay una ventana en la que caerse signifique perder el evento — sin error, sin traza y sin más síntoma que un módulo que no se enteró. **La firma de `EventBus` no cambia y la garantía tampoco**: sigue siendo at-least-once y los handlers siguen teniendo que ser idempotentes, y eso se escribe con estas palabras porque la lectura contraria es cara —alguien retira la idempotencia de un handler y el fallo aparece meses después como una línea de la compra duplicada—. Se resuelven las **dos preguntas** que el plan asignó al hito: el outbox **convive** con la entrega en el acto en lugar de sustituirla —hacerla asíncrona respecto a la petición cambiaría lo que la aplicación garantiza al responder, en los cuatro módulos a la vez— y **la fila entregada se borra**, de modo que la cola vacía es el estado normal y su tamaño es el indicador. La trampa estaba en una línea: el `try/catch` de `publish` **dejaría de proteger al core para tragarse el fallo de escribir la fila**, perdiendo el evento en el silencio exacto que el outbox viene a impedir; la escritura sale del `try`, y un reparto que lanza ya no confirma — con lo que se cierra de paso **la peor limitación conocida del bus**, medida desde la Fase 1: un `@EventListener` a pelo que revienta dejaba sin evento a los handlers que iban detrás, y ahora lo reciben por el relay. `OutboxRelay` reutiliza la forma de `DailySweep` —hogar a hogar, sin `BYPASSRLS`— y no su vehículo: **no es una `ScheduledCheck`**, tiene periodo propio medido en segundos y su propio interruptor, medido en los dos sentidos. La migración `V15` sube el modelo de 28 a **29 tablas** y añade la **sexta** función acotada de `drp_resolver`, que devuelve solo identificadores de los hogares con pendientes. Se decide, con una prueba delante y no razonándolo, que **`AFTER_COMMIT` y `REQUIRES_NEW` se quedan** —lo que cambia de categoría es `fallbackExecution`, sin el cual el camino de recuperación entero no entregaría nada— y que **la guarda de idempotencia se muda a la fila del outbox** y no a una tabla de `(handler, eventId)`, que no cerraría la ventana y que ningún handler desplegado necesita: los tres ya son idempotentes por construcción en sus propias tablas. La prueba que justifica el hito es la que hasta hoy no se podía escribir: **publicar, cortar el proceso antes de repartir, arrancar y comprobar que el handler recibe**, con el módulo de prueba de testigo. Y la batería destapa algo que no se había pensado: **`event_outbox` es la primera tabla del modelo que no puede estar llena** —su fila se borra al repartirse—, así que la prueba de la cascada del Hito 0, que exige que cada tabla con `household_id` tenga algo dentro antes de purgar, la llena **a mano** con una entrega que nadie confirmará. **Sin frontend y sin tocar la batería E2E**, que es la excepción que el plan declaró: nada de lo que hace es visible en una pantalla. Ocho decisiones que la definición no preveía (ver 4.1.7) |
