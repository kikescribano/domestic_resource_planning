...
>
> El bloque añade **cuatro ADR** a las once existentes, y van escribiéndose en el hito que las estrena, como en las dos fases anteriores y no al planificar: supresión de datos (ADR-012, Hito 0, escrita), Transactional Outbox (ADR-013, Hito 1, escrita) y conversión de HEIC (ADR-014, Hito 2, escrita); queda el color e icono elegidos por el usuario dentro de una paleta certificada (ADR-015, Hito 4). **El Hito 3 no tenía ninguna asignada y no se le inventó una**: tres columnas anulables sobre dos tablas que ya existen no dejan ninguna alternativa estructural que descartar, así que sus ocho decisiones están en 4.1.7 y no en una ADR de trámite.
>
