...
> **Fase actual:** Fase 1 (Core MVP) **completada**; cómo se hizo, hito a hito, en [`docs/common/product/roadmap.md`](docs/common/product/roadmap.md). La **Fase 2 está en curso**: los cuatro módulos de prioridad alta de la sección 4.2, sobre activación por hogar y plataforma de avisos, en [`docs/common/product/phase-2-roadmap.md`](docs/common/product/phase-2-roadmap.md). Sus **Hitos 0 a 5 están cerrados**: las fronteras de módulo y la activación por hogar ya existen, el catálogo se puede encender y apagar, la plataforma **programa las comprobaciones periódicas y entrega los avisos** —con lo que los tres procesos diarios del core se ejecutan por fin en un despliegue— y **los cuatro módulos de prioridad alta están construidos**: Proveedores, Warehouse, Compras y Mantenimiento. **Los cuatro riesgos arquitectónicos de la fase están retirados** desde el Hito 4 —incluido el principal, dos módulos que se hablan sin depender uno de que el otro esté activo— y con el Hito 5 **lo único que le queda a la fase es su cierre**: el barrido de aislamiento de las cuarenta y una operaciones nuevas, la auditoría de accesibilidad completa y volver a medir la capacidad
...
El **registro vivo de decisiones** del core: treinta y tres validadas, cada una con su alternativa descartada y el motivo, más tres pendientes que siguen abiertas a propósito —baja de un hogar, análisis antivirus y cuatro atributos propuestos—. La de **peso y volumen de un asset** se resolvió en el Hito 3 de la Fase 2, y su respuesta fue que la pregunta era del core y no del módulo al que estaba asignada.
...
>
> **Los cuatro de prioridad alta son el alcance de la Fase 2**, planificada el 2026-08-18 en [`docs/common/product/phase-2-roadmap.md`](docs/common/product/phase-2-roadmap.md): se construyen sobre el mecanismo de activación por hogar y la plataforma de avisos, que llegan primero. Siguen aquí como "Por diseñar" y pasan a "En desarrollo" **cuando su hito arranca**, no cuando se planifican: el estado de esta tabla es el del módulo, no el del plan.
>
...
Se entrega en **siete hitos**, cada uno atravesando las capas en vertical. **Un hito por sesión de trabajo y un pull request por hito**, sin abrir el siguiente hasta que el anterior esté fusionado. Los dos primeros no llevan funcionalidad de producto: son las fronteras de módulo con su activación, y la plataforma de programación y avisos. **Al cerrar el cuarto la arquitectura está entera y hay tres módulos funcionando** — ese era el punto por el que la fase se partía si hubiera que partirla, y el 2026-08-19 se declaró que no se parte. **El quinto cierra el cuarto módulo y el sexto no añade producto: consolida.**
...
- **Sección 8** — marcar **fases** como en curso o completadas, y añadir nuevas si el roadmap se ajusta. El estado de los **hitos** de la Fase 1 se lleva en [`docs/common/product/roadmap.md`](docs/common/product/roadmap.md), que es también lo que hay que leer para arrancar uno.
...
- **El deck de [`docs/common/marketing/`](docs/common/marketing/README.md)** — resume este documento, y **no avisa cuando se queda atrás**: sigue generándose igual de bien con datos que ya no son ciertos. Estuvo congelado en el estado del 2026-08-07 mientras el README seguía avanzando. Al cambiar algo sustantivo, actualiza `build-drp-resumen.js` y vuelve a ejecutarlo.
