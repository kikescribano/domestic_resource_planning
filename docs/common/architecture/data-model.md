...
|---|---|
| `households` | `time_zone` con un identificador IANA válido; se valida en el caso de uso, no como `CHECK`. No lleva `created_by` ni `updated_by`: cuando la fila nace no existe ningún usuario al que apuntar. Es la columna con la que toda regla de calendario resuelve qué día es hoy en ese hogar; el proceso de vencidos **no** la usa, porque `due_at` es un instante |
