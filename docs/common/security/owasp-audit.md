...
- **Además entra CodeQL**, que el hallazgo citaba entre lo ausente aunque la
  ...
  **cinco alertas de severidad alta, y las cinco se revisaron contra el código
  ...
  Y un modo de fallo de la propia herramienta, ya pagado una vez: **el descarte
  de una alerta no sobrevive a las ediciones vecinas**. Code scanning identifica
  cada alerta por una huella calculada sobre la línea señalada y su contexto,
  así que un cambio en las líneas de al lado —la retirada del CORS añadió su
  bloque justo debajo del `csrf.disable()`— hace que la alerta descartada pase
  a `fixed` y el mismo hallazgo renazca con número nuevo, abierto y sin heredar
  el motivo (la #4 renació como #6 el 2026-08-21). Ante una alerta «nueva» en
  una línea ya revisada, lo primero es buscarla entre las descartadas de esa
  misma ruta: si es una resurrección, se verifica que la premisa del descarte
  siga siendo cierta y se vuelve a descartar citando la original, en lugar de
  tratarla como hallazgo nuevo.
...
| 2026-08-21 | **La alerta del CSRF resucita como #6 y se vuelve a descartar con el mismo motivo.** La retirada del CORS (hallazgos 10/11/19) editó las líneas contiguas al `csrf.disable()` de `SecurityConfig` y cambió la huella con la que code scanning identifica la alerta: la #4 —*won't fix* del 2026-08-20— pasó a `fixed` y el hallazgo renació como #6, abierta y sin heredar el descarte. Antes de re-descartar se verificó que la premisa sigue en pie: ni una cookie en todo el backend, el filtro JWT lee solo la cabecera `Authorization`, la sesión es `STATELESS` y el frontend mantiene los tokens fuera de cookies a propósito. El mecanismo queda descrito en la sección de CodeQL, porque volverá a ocurrir con cualquier edición vecina a una línea señalada: no es un hallazgo nuevo, es el descarte que no viaja con la huella. | kikescribano |
