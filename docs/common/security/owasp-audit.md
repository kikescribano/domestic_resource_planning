...
| 2026-08-20 | **Hallazgo 4 cerrado: fuera la libwebp sin parchear.** `org.sejda.imageio:webp-imageio:0.1.6` —publicada en 2020 y nunca más— se sustituye por `com.github.usefulness:webp-imageio:0.11.0`, la continuación mantenida del mismo código, que empaqueta **libwebp 1.6.0**. Importaba porque ese decodificador se alimenta con los bytes que sube cualquier miembro de cualquier hogar: `image/webp` está en la lista blanca y toda imagen se decodifica para recodificarla. **La versión del nativo se verificó, no se supuso**: el README no la dice, así que se descargó el jar, se comprobó que trae binarios por plataforma y se confirmó contra las notas de publicación del proyecto. El cambio **no toca una sola línea de la aplicación**, porque la sustituta conserva el paquete `com.luciad.imageio.webp` y el procesador pide el códec por SPI genérico. Queda anotado un detalle que solo aparece al desplegar: desde la 0.11.0 los binarios de Linux x86_64 ya no valen para glibc muy antiguas, lo que importaría sobre una imagen musl. **Con esto el único ALTA abierto es el salto a Spring Boot 4.x** | kikescribano |
